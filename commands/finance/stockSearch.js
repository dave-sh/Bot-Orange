...
module.exports = class stockSearch extends Command {
    constructor(client) {
        super(client, {
            ...
            description: 'Searches stock options for best fit.',
            examples: ['c.stocksearch Microsoft'],
            args: [{
                    key: 'symbol',
    ...
    async run(msg, {symbol}) {
        fetch(`https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${symbol}&apikey=${stockAPIKey}`)
            ...
            .then(json => {
               ...
               let url = `https://www.google.com/search?q=${name.replace(/\s+/g, "+") }`;//replaces spaces with + signs for google
