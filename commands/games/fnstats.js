...
module.exports = class FNStats extends Command {
    constructor(client) {
        super(client, {
            ...
            args: [
                ...
                {
                    key: 'platform',
                    prompt: 'Enter a platform xb1, pc, ps4',
                    type: 'string',
                    default: 'pc',
                    validate: platform => {
                        if (platforms.includes(platform.toLowerCase())) return true;
                        return 'Enter a valid platform, \`pc, xb1, ps4\`. Mobile is not currently supported.';
                    },
    ...
    async run(msg, {user, platform}) {
        ...
        .then(async (json) =>{
                ...
                const collector = lifetimeR.createReactionCollector(filter, {time: 100000});
